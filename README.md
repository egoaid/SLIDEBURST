...
録画前にフィルターを選んでおけば、そのフィルターがかかった状態でそのまま録画されます（Hi8・VHS・8mm・ガラケー・映画・モノクロから選べます）。
シャッターを押すと録画が始まり、もう一度押すと止まります（最長60秒）。撮影後の加工はできないので、フィルターは録画前に選んでください。
...
**VHS風の文字** も重ねられます。タイトル文字（フォント・色・大きさ・縁取りを選べます）、左上の「▶ PLAY」表示、右下のテープカウンター（再生・書き出しのあいだ実際に数字が動きます）。当時のビデオデッキがテープに焼き込んでいたような文字を映像に重ねられます。
...
**保存** — できあがったループを**動画**・**GIF**・**静止画**で保存できます。いま画面に表示しているコマ1枚を、フィルターやスタンプごと写真として保存したいときは静止画を選んでください。
